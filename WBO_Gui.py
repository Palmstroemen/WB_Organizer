...
def openMyWB():
    ...
    if not os.path.exists(__pathGroupFile__):
        ...
        __groupedWB__ = {__strAll__: __justWB__, __strNew__: __justWB__, __strLost__: [], __strDrop__: [], __strDisabled__: []}
        ...
        with open(__pathGroupFile__, 'r') as f:
            ...
            if not loadedAll: loadedAll = {}  # prevents error in case __strAll__ has been changed
...
def openFileInEditor():
    ...
    try:
        if sys.platform.startswith('linux'):
            subprocess.Popen(['nano', __pathGroupFile__])
        elif sys.platform.startswith('win'):
            subprocess.Popen(['notepad', __pathGroupFile__])
        elif sys.platform.startswith('darwin'):  # MacOSX
            subprocess.Popen(['open', '-t', __pathGroupFile__])
        else:
...
def prefDialog():
    ...
    l5.addWidget(mg1)            # Groups funktioniert noch nicht
